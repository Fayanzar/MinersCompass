...
public class DirectionLines {
    public static void drawDirections (WorldRenderContext context) {
        ...
        var trackedPositions = OreCompass.getTrackedPos(nbt);
        if (trackedPositions == null) return;
        ...
        for (var trackedPos : trackedPositions) {
            if (trackedPos == null) continue;

            var block = world.getBlockState(trackedPos.pos()).getBlock();
            var oreType = DynamicOreType.fromBlock(block);
            var formatting = Formatting.RED;
            if (oreType != null) {
                formatting = oreType.getColor();
            }
